...
def match_entities(graph, entity_text):
  candidates = []
  entity_dec = entity_text.split(" ")
  if graph.get_entity(entity_text):
    candidates.append(entity_text)
  for i in range(0, len(entity_dec), 1):
    subword = entity_dec[i:i + 1]
    if graph.get_entity(subword):
      candidates.append(subword)
  for i in range(0, len(entity_dec), 2):
    subword = entity_dec[i:i + 2]
    if graph.get_entity(subword):
      candidates.append(subword)
  for i in range(0, len(entity_dec), 3):
    subword = entity_dec[i:i + 3]
    if graph.get_entity(subword):
      candidates.append(subword)
  return candidates
