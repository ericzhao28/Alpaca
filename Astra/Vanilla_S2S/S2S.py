...
class S2SAgent(Agent):
    ...
    def add_cmdline_args(argparser):
        ...
        agent.add_argument('-hs', '--hiddensize', type=int, default=constants.HIDDEN_SIZE,
            help='size of the hidden layers and embeddings')
        agent.add_argument('-nl', '--numlayers', type=int, default=constants.NUM_LAYERS,
            help='number of hidden layers')
        agent.add_argument('-lr', '--learningrate', type=float, default=constants.LEARNING_RATE,
            help='learning rate')
        agent.add_argument('-enc_dr', '--encoder_dropout', type=float, default=constants.ENCODER_DROPOUT,
            help='output dropout rate')
        agent.add_argument('-dec_dr', '--decoder_dropout', type=float, default=constants.DECODER_DROPOUT,
            help='output dropout rate')
        agent.add_argument('-out_dr', '--h2o_dropout', type=float, default=constants.H2O_DROPOUT,
            help='output dropout rate')
        # agent.add_argument('-bi', '--bidirectional', type='bool', default=False,
        #     help='whether to encode the context with a bidirectional RNN')
        agent.add_argument('--no-cuda', action='store_true', default=constants.NO_CUDA,
        ...
        agent.add_argument('-rc', '--rank-candidates', type='bool', default=False,
            help='rank candidates if available. this is done by computing the' +
                 ...
                 'highest scoring one.')
        agent.add_argument('--clip', type=float, default=constants.GRADIENT_CLIP,
            help='gradient clipping')

        agent.add_argument('--beam_size', type=float, default=constants.BEAM_SIZE,
            help='Beam size')
    ...
    def __init__(self, opt, shared=None):
        ...
        if not shared:
            ...
            self.encoder = nn.GRU(hsz, hsz, opt['numlayers'], dropout=opt['encoder_dropout'])
            # decoder produces our output states
            self.decoder = nn.GRU(hsz, hsz, opt['numlayers'], dropout=opt['decoder_dropout'])
            ...
            self.dropout = nn.Dropout(opt['h2o_dropout'])
    ...
    def hidden_to_idx(self, hidden, dropout=True):
    ...
    def observe(self, observation):
        ...
        if self.observation and not self.episode_done:
    ...
    def predict(self, xs, ys=None, cands=None):
        ...
        if ys is not None:
            ...
            loss.backward()
            nn.utils.clip_grad_norm(self.encoder.parameters(), self.clip_val)
            nn.utils.clip_grad_norm(self.decoder.parameters(), self.clip_val)
            self.update_params()
            if random.random() < 0.1:
                # sometimes output a prediction for debugging
                print('================================================================================')
                print('query: ', self.dict.vec2txt(xs.data[0]))
                print('prediction:', ' '.join(output_lines[0]))
                print('label:', self.dict.vec2txt(ys.data[0]))
    ...
    def act(self):
        ...
        x = self.batch_act([self.observation])
        return x[0]
